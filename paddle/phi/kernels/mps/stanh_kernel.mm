...
template <typename T>
void StanhKernelImpl(const MPSContext& dev_ctx,
                     const DenseTensor& x,
                     float scale_a,
                     float scale_b,
                     DenseTensor* out) {
  @autoreleasepool {
    ...
    // stanh(x) = scale_b * tanh(scale_a * x)
    MPSGraphTensor* scale_a_tensor =
        [graph constantWithScalar:static_cast<float>(scale_a)
                            shape:@[@1]
                         dataType:backends::mps::GetMPSDataType(x.dtype())];
    MPSGraphTensor* scale_b_tensor =
        [graph constantWithScalar:static_cast<float>(scale_b)
                            shape:@[@1]
                         dataType:backends::mps::GetMPSDataType(x.dtype())];
    MPSGraphTensor* scaled_x =
        [graph multiplicationWithPrimaryTensor:scale_a_tensor
                               secondaryTensor:x_tensor
                                          name:@"stanh_scaled_x"];
    MPSGraphTensor* tanh_t = [graph tanhWithTensor:scaled_x
                                              name:@"stanh_tanh"];
    MPSGraphTensor* result_tensor =
        [graph multiplicationWithPrimaryTensor:scale_b_tensor
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for stanh";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for stanh";
      return;
    }
...
void StanhKernel(const Context& dev_ctx,
                 const DenseTensor& x,
                 float scale_a,
                 float scale_b,
                 DenseTensor* out) {
  ...
  if (mps_ctx != nullptr) {
    StanhKernelImpl<T>(*mps_ctx, x, scale_a, scale_b, out);
  } else {
