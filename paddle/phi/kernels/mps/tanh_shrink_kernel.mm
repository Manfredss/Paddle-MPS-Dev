...
template <typename T>
void TanhShrinkKernelImpl(const MPSContext& dev_ctx,
                          const DenseTensor& x,
                          DenseTensor* out) {
  @autoreleasepool {
    ...
    // tanh_shrink(x) = x - tanh(x)
    MPSGraphTensor* tanh_x = [graph tanhWithTensor:x_tensor
                                              name:@"tanh_shrink_tanh"];
    MPSGraphTensor* result_tensor =
        [graph subtractionWithPrimaryTensor:x_tensor
                            secondaryTensor:tanh_x
                                       name:@"tanh_shrink_result"];

    dev_ctx.template Alloc<T>(out);

    id<MTLBuffer> out_buffer = backends::mps::GetMTLBuffer(*out);
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for tanh_shrink";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for tanh_shrink";
      return;
    }
...
void TanhShrinkKernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    TanhShrinkKernelImpl<T>(*mps_ctx, x, out);
  } else {
...
PD_REGISTER_KERNEL(tanh_shrink,
                   MPS,
                   ALL_LAYOUT,
