...
void RemainderKernelImpl(const MPSContext& dev_ctx,
  ...
  @autoreleasepool {
    ...
    // Floating-point remainder with the sign of the divisor (Python/paddle
    // semantics): r = x - floor(x / y) * y.
    MPSGraphTensor* div = [graph divisionWithPrimaryTensor:x_tensor
                                          secondaryTensor:y_tensor
                                                     name:@"remainder_div"];
    MPSGraphTensor* floor_div = [graph floorWithTensor:div
                                                  name:@"remainder_floor"];
    MPSGraphTensor* prod = [graph multiplicationWithPrimaryTensor:floor_div
                                                 secondaryTensor:y_tensor
                                                            name:@"remainder_mul"];
    ...
    if (x_buffer == nil || y_buffer == nil) {
      VLOG(3) << "Input buffers not available, using CPU fallback for remainder";
      return;
    }
