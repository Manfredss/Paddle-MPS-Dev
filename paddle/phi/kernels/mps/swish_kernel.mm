...
template <typename T>
void SwishKernelImpl(const MPSContext& dev_ctx,
                     const DenseTensor& x,
                     DenseTensor* out) {
  @autoreleasepool {
    ...
    // swish(x) = x / (1 + exp(-x)) (beta fixed to 1, matching phi::SwishKernel).
    ...
    MPSGraphTensor* exp_neg_x = [graph exponentWithTensor:neg_x
                                                     name:@"swish_exp"];
    MPSGraphTensor* denom = [graph additionWithPrimaryTensor:one
                                            secondaryTensor:exp_neg_x
                                                       name:@"swish_denom"];
    MPSGraphTensor* result_tensor = [graph divisionWithPrimaryTensor:x_tensor
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for swish";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for swish";
      return;
    }
...
void SwishKernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    SwishKernelImpl<T>(*mps_ctx, x, out);
  } else {
