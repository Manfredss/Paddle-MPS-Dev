...
template <typename T>
void AsinhKernelImpl(const MPSContext& dev_ctx,
                     const DenseTensor& x,
                     DenseTensor* out) {
  @autoreleasepool {
    ...
    // y = asinh(x) using MPSGraph asinhWithTensor:
    MPSGraphTensor* result_tensor = [graph asinhWithTensor:x_tensor
                                                      name:@"asinh_result"];
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for asinh";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for asinh";
      return;
    }
...
void AsinhKernel(const Context& dev_ctx,
                 const DenseTensor& x,
                 DenseTensor* out) {
  if (x.numel() == 0) {
    dev_ctx.template Alloc<T>(out);
  ...
  if (mps_ctx != nullptr) {
    AsinhKernelImpl<T>(*mps_ctx, x, out);
  } else {
