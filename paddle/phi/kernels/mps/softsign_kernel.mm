...
template <typename T>
void SoftsignKernelImpl(const MPSContext& dev_ctx,
                        const DenseTensor& x,
                        DenseTensor* out) {
  @autoreleasepool {
    ...
    MPSGraphTensor* result_tensor =
        [graph divisionWithPrimaryTensor:x_tensor
                         secondaryTensor:denom
                                    name:@"softsign_result"];

    dev_ctx.template Alloc<T>(out);

    id<MTLBuffer> out_buffer = backends::mps::GetMTLBuffer(*out);
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for softsign";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for softsign";
      return;
    }
...
void SoftsignKernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    SoftsignKernelImpl<T>(*mps_ctx, x, out);
  } else {
