...
template <typename T>
void MeanKernelImpl(const MPSContext& dev_ctx,
                    const DenseTensor& x,
                    const IntArray& dims,
                    bool keep_dim,
                    DenseTensor* out) {
  @autoreleasepool {
    ...
    MPSGraphTensor* reduced = [graph meanOfTensor:x_tensor
                                             axes:axes
                                             name:@"mean_reduce"];

    // MPSGraph reductions keep reduced axes as size 1. Reshape to match
    // the Paddle-provided output shape (which already accounts for keep_dim).
    auto out_dims = out->dims();
    NSMutableArray<NSNumber*>* out_shape = [NSMutableArray array];
    if (out_dims.size() == 0) {
      // Scalar output: represent as shape [1] for MPSGraph.
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for mean";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for mean";
      return;
    }
...
void MeanKernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    MeanKernelImpl<T>(*mps_ctx, x, dims, keep_dim, out);
  } else {
