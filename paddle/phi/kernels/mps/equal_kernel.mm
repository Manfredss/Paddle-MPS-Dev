...
template <typename T>
void EqualKernelImpl(const MPSContext& dev_ctx,
                     const DenseTensor& x,
                     const DenseTensor& y,
                     DenseTensor* out) {
  @autoreleasepool {
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for equal";
      return;
    }
    ...
    if (x_buffer == nil || y_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for equal";
      return;
    }
...
void EqualKernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    EqualKernelImpl<T>(*mps_ctx, x, y, out);
  } else {
