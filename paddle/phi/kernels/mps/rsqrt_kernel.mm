...
template <typename T>
void RsqrtKernelImpl(const MPSContext& dev_ctx,
                     const DenseTensor& x,
                     DenseTensor* out) {
  @autoreleasepool {
    ...
    // y = 1 / sqrt(x) using MPSGraph reverseSquareRootWithTensor:
    MPSGraphTensor* result_tensor = [graph reverseSquareRootWithTensor:x_tensor
                                                                  name:@"rsqrt_result"];
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for rsqrt";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for rsqrt";
      return;
    }
...
void RsqrtKernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    RsqrtKernelImpl<T>(*mps_ctx, x, out);
  } else {
