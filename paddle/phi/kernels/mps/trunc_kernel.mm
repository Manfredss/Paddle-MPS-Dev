...
void TruncKernelImpl(const MPSContext& dev_ctx,
  ...
  @autoreleasepool {
    ...
    // y = trunc(x) (round toward zero) using MPSGraph truncateWithTensor:
    MPSGraphTensor* result_tensor = [graph truncateWithTensor:x_tensor
                                                         name:@"trunc_result"];
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS output buffer not available for trunc; skipping graph run";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "MPS input buffer not available for trunc; skipping graph run";
      return;
    }
