...
template <typename T>
void TruncKernelImpl(const MPSContext& dev_ctx,
                     const DenseTensor& x,
                     DenseTensor* out) {
  @autoreleasepool {
    ...
    // trunc(x) rounds toward zero: ceil(x) for x < 0, floor(x) otherwise.
    ...
    MPSGraphTensor* is_neg = [graph lessThanWithPrimaryTensor:x_tensor
                                             secondaryTensor:zero
                                                        name:@"trunc_is_neg"];
    MPSGraphTensor* ceil_x = [graph ceilWithTensor:x_tensor name:@"trunc_ceil"];
    MPSGraphTensor* floor_x = [graph floorWithTensor:x_tensor name:@"trunc_floor"];
    MPSGraphTensor* result_tensor =
        [graph selectWithPredicateTensor:is_neg
                     truePredicateTensor:ceil_x
                    falsePredicateTensor:floor_x
                                    name:@"trunc_result"];
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for trunc";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for trunc";
      return;
    }
...
void TruncKernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    TruncKernelImpl<T>(*mps_ctx, x, out);
  } else {
