...
template <typename T>
void AcoshKernelImpl(const MPSContext& dev_ctx,
                     const DenseTensor& x,
                     DenseTensor* out) {
  @autoreleasepool {
    ...
    // y = acosh(x) = log(x + sqrt(x^2 - 1)); domain requires x >= 1.
    MPSGraphTensor* result_tensor = [graph acoshWithTensor:x_tensor
                                                      name:@"acosh_result"];
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for acosh";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for acosh";
      return;
    }
...
void AcoshKernel(const Context& dev_ctx,
                 const DenseTensor& x,
                 DenseTensor* out) {
  if (x.numel() == 0) {
    dev_ctx.template Alloc<T>(out);
  ...
  if (mps_ctx != nullptr) {
    AcoshKernelImpl<T>(*mps_ctx, x, out);
  } else {
