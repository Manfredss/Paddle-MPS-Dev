...
template <typename T>
void LogSigmoidKernelImpl(const MPSContext& dev_ctx,
                          const DenseTensor& x,
                          DenseTensor* out) {
  @autoreleasepool {
    ...
    // Numerically stable log-sigmoid, matching phi::funcs::LogSigmoidFunctor:
    //   temp = max(-x, 0)
    //   y    = -temp - log(exp(-temp) + exp(-x - temp))
    MPSGraphTensor* zero = [graph constantWithScalar:0.0f
                                               shape:@[@1]
                                            dataType:MPSDataTypeFloat32];
    MPSGraphTensor* neg_x = [graph negativeWithTensor:x_tensor
                                                 name:@"logsigmoid_neg_x"];
    MPSGraphTensor* temp = [graph maximumWithPrimaryTensor:neg_x
                                          secondaryTensor:zero
                                                     name:@"logsigmoid_temp"];
    MPSGraphTensor* neg_temp = [graph negativeWithTensor:temp
                                                    name:@"logsigmoid_neg_temp"];
    MPSGraphTensor* exp_neg_temp = [graph exponentWithTensor:neg_temp
                                                        name:@"logsigmoid_exp1"];
    MPSGraphTensor* neg_x_minus_temp =
        [graph subtractionWithPrimaryTensor:neg_x
                            secondaryTensor:temp
                                       name:@"logsigmoid_nx_minus_temp"];
    MPSGraphTensor* exp_neg_x_minus_temp =
        [graph exponentWithTensor:neg_x_minus_temp name:@"logsigmoid_exp2"];
    MPSGraphTensor* sum_exp =
        [graph additionWithPrimaryTensor:exp_neg_temp
                         secondaryTensor:exp_neg_x_minus_temp
                                    name:@"logsigmoid_sum"];
    MPSGraphTensor* log_sum = [graph logarithmWithTensor:sum_exp
                                                    name:@"logsigmoid_log"];
    MPSGraphTensor* result_tensor =
        [graph subtractionWithPrimaryTensor:neg_temp
                            secondaryTensor:log_sum
                                       name:@"logsigmoid_result"];

    dev_ctx.template Alloc<T>(out);

    id<MTLBuffer> out_buffer = backends::mps::GetMTLBuffer(*out);
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for logsigmoid";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for logsigmoid";
      return;
    }
...
void LogSigmoidKernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    LogSigmoidKernelImpl<T>(*mps_ctx, x, out);
  } else {
