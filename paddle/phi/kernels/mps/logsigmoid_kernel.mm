...
void LogSigmoidKernelImpl(const MPSContext& dev_ctx,
  ...
  @autoreleasepool {
    ...
    // Numerically stable form: min(x, 0) - log(1 + exp(-|x|))
    // (avoids overflow of exp for large negative x)
    ...
    MPSGraphTensor* min_x_zero =
        [graph minimumWithPrimaryTensor:x_tensor
                        secondaryTensor:zero
                                   name:@"logsigmoid_min_x_zero"];
    MPSGraphTensor* abs_x = [graph absoluteWithTensor:x_tensor
                                                 name:@"logsigmoid_abs_x"];
    MPSGraphTensor* neg_abs_x = [graph negativeWithTensor:abs_x
                                                     name:@"logsigmoid_neg_abs_x"];
    MPSGraphTensor* exp_neg_abs =
        [graph exponentWithTensor:neg_abs_x name:@"logsigmoid_exp_neg_abs"];
    MPSGraphTensor* one_plus_exp =
        [graph additionWithPrimaryTensor:one
                         secondaryTensor:exp_neg_abs
                                    name:@"logsigmoid_one_plus_exp"];
    MPSGraphTensor* log_term =
        [graph logarithmWithTensor:one_plus_exp name:@"logsigmoid_log_term"];
    MPSGraphTensor* result_tensor =
        [graph subtractionWithPrimaryTensor:min_x_zero
                            secondaryTensor:log_term
