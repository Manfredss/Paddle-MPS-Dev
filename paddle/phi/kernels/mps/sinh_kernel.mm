...
template <typename T>
void SinhKernelImpl(const MPSContext& dev_ctx,
                    const DenseTensor& x,
                    DenseTensor* out) {
  @autoreleasepool {
    ...
    // y = sinh(x) using MPSGraph sinhWithTensor:
    MPSGraphTensor* result_tensor = [graph sinhWithTensor:x_tensor
                                                     name:@"sinh_result"];
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for sinh";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for sinh";
      return;
    }
...
void SinhKernel(const Context& dev_ctx,
                const DenseTensor& x,
                DenseTensor* out) {
  if (x.numel() == 0) {
    dev_ctx.template Alloc<T>(out);
  ...
  if (mps_ctx != nullptr) {
    SinhKernelImpl<T>(*mps_ctx, x, out);
  } else {
