...
template <typename T>
void Expm1KernelImpl(const MPSContext& dev_ctx,
                     const DenseTensor& x,
                     DenseTensor* out) {
  @autoreleasepool {
    ...
    // expm1(x) = exp(x) - 1, composed from exponent and subtraction.
    MPSGraphTensor* exp_tensor = [graph exponentWithTensor:x_tensor
                                                      name:@"expm1_exp"];
    ...
    MPSGraphTensor* result_tensor =
        [graph subtractionWithPrimaryTensor:exp_tensor
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for expm1";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for expm1";
      return;
    }
...
void Expm1Kernel(const Context& dev_ctx,
                 const DenseTensor& x,
                 DenseTensor* out) {
  if (x.numel() == 0) {
    dev_ctx.template Alloc<T>(out);
  ...
  if (mps_ctx != nullptr) {
    Expm1KernelImpl<T>(*mps_ctx, x, out);
  } else {
