...
template <typename T>
void SoftmaxKernelImpl(const MPSContext& dev_ctx,
                       const DenseTensor& x,
                       int axis,
                       DenseTensor* out) {
  @autoreleasepool {
    ...
    // Paddle accepts negative axis; MPSGraph requires non-negative.
    int normalized_axis = axis < 0 ? axis + rank : axis;
    PADDLE_ENFORCE_GE(normalized_axis,
                      0,
                      common::errors::InvalidArgument(
                          "softmax axis out of range: %d for rank %d", axis, rank));
    ...
    MPSGraphTensor* result_tensor = [graph softMaxWithTensor:x_tensor
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for softmax";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for softmax";
      return;
    }
...
void SoftmaxKernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    SoftmaxKernelImpl<T>(*mps_ctx, x, axis, out);
  } else {
