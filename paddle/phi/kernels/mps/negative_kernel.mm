...
template <typename T>
void NegativeKernelImpl(const MPSContext& dev_ctx,
                        const DenseTensor& x,
                        DenseTensor* out) {
  @autoreleasepool {
    ...
    // y = -x using MPSGraph negativeWithTensor:
    MPSGraphTensor* result_tensor = [graph negativeWithTensor:x_tensor
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for negative";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for negative";
      return;
    }
...
void NegativeKernel(const Context& dev_ctx,
                    const DenseTensor& x,
                    DenseTensor* out) {
  if (x.numel() == 0) {
    dev_ctx.template Alloc<T>(out);
  ...
  if (mps_ctx != nullptr) {
    NegativeKernelImpl<T>(*mps_ctx, x, out);
  } else {
