...
template <typename T>
void SeluKernelImpl(const MPSContext& dev_ctx,
                    const DenseTensor& x,
                    float scale,
                    float alpha,
                    DenseTensor* out) {
  @autoreleasepool {
    ...
    // selu(x) = scale * (x if x > 0, else alpha * exp(x) - alpha)
    MPSGraphTensor* scale_tensor =
        [graph constantWithScalar:static_cast<float>(scale)
                            shape:@[@1]
                         dataType:MPSDataTypeFloat32];
    MPSGraphTensor* alpha_tensor =
        [graph constantWithScalar:static_cast<float>(alpha)
                            shape:@[@1]
                         dataType:MPSDataTypeFloat32];
    MPSGraphTensor* zero = [graph constantWithScalar:0.0f
                                               shape:@[@1]
                                            dataType:MPSDataTypeFloat32];
    MPSGraphTensor* exp_x = [graph exponentWithTensor:x_tensor
                                                 name:@"selu_exp"];
    MPSGraphTensor* alpha_exp =
        [graph multiplicationWithPrimaryTensor:alpha_tensor
                               secondaryTensor:exp_x
                                          name:@"selu_alpha_exp"];
    MPSGraphTensor* neg_branch =
        [graph subtractionWithPrimaryTensor:alpha_exp
                            secondaryTensor:alpha_tensor
                                       name:@"selu_neg_branch"];
    MPSGraphTensor* pred =
        [graph lessThanOrEqualToWithPrimaryTensor:x_tensor
                                  secondaryTensor:zero
                                             name:@"selu_pred"];
    MPSGraphTensor* selected =
        [graph selectWithPredicateTensor:pred
                     truePredicateTensor:neg_branch
                    falsePredicateTensor:x_tensor
                                    name:@"selu_selected"];
    MPSGraphTensor* result_tensor =
        [graph multiplicationWithPrimaryTensor:scale_tensor
                               secondaryTensor:selected
                                          name:@"selu_result"];
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for selu";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for selu";
      return;
    }
...
void SeluKernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    SeluKernelImpl<T>(*mps_ctx, x, scale, alpha, out);
  } else {
