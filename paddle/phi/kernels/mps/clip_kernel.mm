...
template <typename T>
void ClipKernelImpl(const MPSContext& dev_ctx,
                    const DenseTensor& x,
                    const Scalar& min,
                    const Scalar& max,
                    DenseTensor* out) {
  @autoreleasepool {
    ...
    MPSGraphTensor* result_tensor =
        [graph clampWithTensor:x_tensor
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for clip";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for clip";
      return;
    }
...
void ClipKernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    ClipKernelImpl<T>(*mps_ctx, x, min, max, out);
  } else {
