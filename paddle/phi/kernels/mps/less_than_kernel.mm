...
template <typename T>
void LessThanKernelImpl(const MPSContext& dev_ctx,
                        const DenseTensor& x,
                        const DenseTensor& y,
                        DenseTensor* out) {
  @autoreleasepool {
    ...
    MPSGraphTensor* result_tensor =
        [graph lessThanWithPrimaryTensor:x_tensor
                          secondaryTensor:y_tensor
                                     name:@"less_than_result"];

    dev_ctx.template Alloc<bool>(out);

    id<MTLBuffer> out_buffer = backends::mps::GetMTLBuffer(*out);
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for less_than";
      return;
    }
    ...
    if (x_buffer == nil || y_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for less_than";
      return;
    }
...
void LessThanKernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    LessThanKernelImpl<T>(*mps_ctx, x, y, out);
  } else {
