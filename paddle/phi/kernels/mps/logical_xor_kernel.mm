...
template <typename T>
void LogicalXorKernelImpl(const MPSContext& dev_ctx,
                          const DenseTensor& x,
                          const DenseTensor& y,
                          DenseTensor* out) {
  @autoreleasepool {
    ...
    MPSGraphTensor* result_tensor =
        [graph logicalXORWithPrimaryTensor:x_tensor
                          secondaryTensor:y_tensor
                                     name:@"logical_xor_result"];

    dev_ctx.template Alloc<bool>(out);

    id<MTLBuffer> out_buffer = backends::mps::GetMTLBuffer(*out);
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for logical_xor";
      return;
    }
    ...
    if (x_buffer == nil || y_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for logical_xor";
      return;
    }
...
void LogicalXorKernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    LogicalXorKernelImpl<T>(*mps_ctx, x, y, out);
  } else {
...
PD_REGISTER_KERNEL(logical_xor,
                   MPS,
                   ALL_LAYOUT,
                   phi::LogicalXorKernel,
                   bool) {
