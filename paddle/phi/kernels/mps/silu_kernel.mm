...
template <typename T>
void SiluKernelImpl(const MPSContext& dev_ctx,
                    const DenseTensor& x,
                    DenseTensor* out) {
  @autoreleasepool {
    ...
    // SiLU(x) = x * sigmoid(x), matches torch.nn.functional.silu / paddle.nn.functional.silu.
    MPSGraphTensor* sigmoid_tensor = [graph sigmoidWithTensor:x_tensor
                                                         name:@"silu_sigmoid"];
    MPSGraphTensor* result_tensor =
        [graph multiplicationWithPrimaryTensor:x_tensor
                               secondaryTensor:sigmoid_tensor
                                          name:@"silu_result"];
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for silu";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for silu";
      return;
    }
...
void SiluKernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    SiluKernelImpl<T>(*mps_ctx, x, out);
  } else {
