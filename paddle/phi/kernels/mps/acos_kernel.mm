...
template <typename T>
void AcosKernelImpl(const MPSContext& dev_ctx,
                    const DenseTensor& x,
                    DenseTensor* out) {
  @autoreleasepool {
    ...
    // y = acos(x) using MPSGraph acosWithTensor:
    MPSGraphTensor* result_tensor = [graph acosWithTensor:x_tensor
                                                     name:@"acos_result"];
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for acos";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for acos";
      return;
    }
...
void AcosKernel(const Context& dev_ctx,
                const DenseTensor& x,
                DenseTensor* out) {
  if (x.numel() == 0) {
    dev_ctx.template Alloc<T>(out);
  ...
  if (mps_ctx != nullptr) {
    AcosKernelImpl<T>(*mps_ctx, x, out);
  } else {
