...
template <typename T>
void SinKernelImpl(const MPSContext& dev_ctx,
                   const DenseTensor& x,
                   DenseTensor* out) {
  @autoreleasepool {
    ...
    // y = sin(x) using MPSGraph sinWithTensor:
    MPSGraphTensor* result_tensor = [graph sinWithTensor:x_tensor
                                                    name:@"sin_result"];
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for sin";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for sin";
      return;
    }
