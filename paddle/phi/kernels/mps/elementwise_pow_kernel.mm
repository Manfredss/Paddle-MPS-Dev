...
template <typename T>
void ElementwisePowKernelImpl(const MPSContext& dev_ctx,
                               const DenseTensor& x,
                               const DenseTensor& y,
                               DenseTensor* out) {
  @autoreleasepool {
    ...
    // Perform power operation using MPSGraph: pow(x, y) = x^y
    // MPSGraph uses powerWithPrimaryTensor:secondaryTensor: for x^y
    MPSGraphTensor* result_tensor = [graph powerWithPrimaryTensor:x_tensor
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for pow";
      return;
    }
    ...
    if (x_buffer == nil || y_buffer == nil) {
      VLOG(3) << "Input buffers not available, using CPU fallback for pow";
      return;
    }
...
void ElementwisePowKernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    ElementwisePowKernelImpl<T>(*mps_ctx, x, y, out);
  } else {
