...
template <typename T>
void MishKernelImpl(const MPSContext& dev_ctx,
                    const DenseTensor& x,
                    float threshold,
                    DenseTensor* out) {
  @autoreleasepool {
    ...
    // mish(x) = x * tanh(softplus(x)), where softplus uses a linear
    // approximation (softplus(x) ~= x) once x exceeds `threshold` to avoid
    // exp() overflow. Matches phi::funcs::MishFunctor.
    ...
    MPSGraphTensor* thr = [graph constantWithScalar:threshold
                                              shape:@[@1]
                                           dataType:MPSDataTypeFloat32];
    MPSGraphTensor* exp_x = [graph exponentWithTensor:x_tensor name:@"mish_exp"];
    MPSGraphTensor* one_plus = [graph additionWithPrimaryTensor:one
                                               secondaryTensor:exp_x
                                                          name:@"mish_1p"];
    MPSGraphTensor* softplus = [graph logarithmWithTensor:one_plus
                                                     name:@"mish_softplus"];
    MPSGraphTensor* is_big = [graph greaterThanWithPrimaryTensor:x_tensor
                                                secondaryTensor:thr
                                                           name:@"mish_is_big"];
    MPSGraphTensor* sp = [graph selectWithPredicateTensor:is_big
                                     truePredicateTensor:x_tensor
                                    falsePredicateTensor:softplus
                                                    name:@"mish_sp"];
    MPSGraphTensor* tanh_sp = [graph tanhWithTensor:sp name:@"mish_tanh"];
    MPSGraphTensor* result_tensor = [graph multiplicationWithPrimaryTensor:x_tensor
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for mish";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for mish";
      return;
    }
...
void MishKernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    MishKernelImpl<T>(*mps_ctx, x, threshold, out);
  } else {
