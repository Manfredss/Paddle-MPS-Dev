...
template <typename T>
void Atan2KernelImpl(const MPSContext& dev_ctx,
                     const DenseTensor& x,
                     const DenseTensor& y,
                     DenseTensor* out) {
  @autoreleasepool {
    ...
    // atan2(x, y) = angle, in radians, of the point (y, x): the primary tensor
    // is the numerator, matching paddle.atan2 / numpy.arctan2 conventions.
    MPSGraphTensor* result_tensor = [graph atan2WithPrimaryTensor:x_tensor
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for atan2";
      return;
    }
    ...
    if (x_buffer == nil || y_buffer == nil) {
      VLOG(3) << "Input buffers not available, using CPU fallback for atan2";
      return;
    }
...
void Atan2Kernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    Atan2KernelImpl<T>(*mps_ctx, x, y, out);
  } else {
