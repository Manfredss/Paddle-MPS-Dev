...
void Atan2KernelImpl(const MPSContext& dev_ctx,
  ...
  @autoreleasepool {
    ...
    // atan2(x, y): x is the numerator, y is the denominator, matching the
    // CPU functor (::atan2f(x, y)). MPSGraph's atan2 takes the numerator as
    // the primary tensor.
    MPSGraphTensor* result_tensor =
        [graph atan2WithPrimaryTensor:x_tensor
    ...
    if (x_buffer == nil || y_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for atan2";
      return;
    }
