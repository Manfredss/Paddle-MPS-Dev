...
template <typename T>
void FMinKernelImpl(const MPSContext& dev_ctx,
                    const DenseTensor& x,
                    const DenseTensor& y,
                    DenseTensor* out) {
  @autoreleasepool {
    ...
    if (is_int) {
      ...
      MPSGraphTensor* y_nan_fixed =
          [graph selectWithPredicateTensor:y_is_nan
                      ...
                      falsePredicateTensor:min_tensor
                                      name:@"fmin_y_nan_fixed"];
      result_tensor =
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for fmin";
      return;
    }
    ...
    if (x_buffer == nil || y_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for fmin";
      return;
    }
...
void FMinKernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    FMinKernelImpl<T>(*mps_ctx, x, y, out);
  } else {
