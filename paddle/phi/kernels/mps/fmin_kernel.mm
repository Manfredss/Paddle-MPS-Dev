...
void FMinKernelImpl(const MPSContext& dev_ctx,
  ...
  @autoreleasepool {
    ...
    // fmin(x, y): elementwise minimum ignoring NaN operands (std::fmin).
    // MPSGraph's plain minimum matches this NaN-suppressing behavior.
    MPSGraphTensor* result_tensor = [graph minimumWithPrimaryTensor:x_tensor
    ...
    if (x_buffer == nil || y_buffer == nil) {
      VLOG(3) << "Input buffers not available, using CPU fallback for fmin";
      return;
    }
