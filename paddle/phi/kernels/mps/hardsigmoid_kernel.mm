...
void HardSigmoidKernelImpl(const MPSContext& dev_ctx,
  ...
  @autoreleasepool {
    ...
    // hardsigmoid(x) = min(max(x * slope + offset, 0), 1).
    ...
    MPSGraphTensor* one = [graph constantWithScalar:1.0f
                                              shape:@[@1]
                                           dataType:MPSDataTypeFloat32];
    MPSGraphTensor* slope_t = [graph constantWithScalar:slope
                                                  shape:@[@1]
                                               dataType:MPSDataTypeFloat32];
    MPSGraphTensor* offset_t = [graph constantWithScalar:offset
                                                   shape:@[@1]
                                                dataType:MPSDataTypeFloat32];
    MPSGraphTensor* scaled = [graph multiplicationWithPrimaryTensor:x_tensor
                                                   secondaryTensor:slope_t
                                                              name:@"hardsigmoid_scale"];
    MPSGraphTensor* shifted = [graph additionWithPrimaryTensor:scaled
                                              secondaryTensor:offset_t
                                                         name:@"hardsigmoid_shift"];
    MPSGraphTensor* clamped_low = [graph maximumWithPrimaryTensor:shifted
                                                 secondaryTensor:zero
                                                            name:@"hardsigmoid_max"];
    MPSGraphTensor* result_tensor = [graph minimumWithPrimaryTensor:clamped_low
                                                   secondaryTensor:one
