...
template <typename T>
void HardSigmoidKernelImpl(const MPSContext& dev_ctx,
                           const DenseTensor& x,
                           float slope,
                           float offset,
                           DenseTensor* out) {
  @autoreleasepool {
    ...
    // hard_sigmoid(x) = min(max(slope * x + offset, 0), 1)
    ...
    MPSGraphTensor* one = [graph constantWithScalar:1.0f
                                              shape:@[@1]
                                           dataType:MPSDataTypeFloat32];
    MPSGraphTensor* slope_tensor =
        [graph constantWithScalar:static_cast<float>(slope)
                            shape:@[@1]
                         dataType:MPSDataTypeFloat32];
    MPSGraphTensor* offset_tensor =
        [graph constantWithScalar:static_cast<float>(offset)
                            shape:@[@1]
                         dataType:MPSDataTypeFloat32];

    MPSGraphTensor* slope_x =
        [graph multiplicationWithPrimaryTensor:slope_tensor
                               secondaryTensor:x_tensor
                                          name:@"hardsigmoid_slope_x"];
    MPSGraphTensor* linear =
        [graph additionWithPrimaryTensor:slope_x
                         secondaryTensor:offset_tensor
                                    name:@"hardsigmoid_linear"];
    MPSGraphTensor* max_zero =
        [graph maximumWithPrimaryTensor:linear
                        secondaryTensor:zero
                                   name:@"hardsigmoid_max_zero"];
    MPSGraphTensor* result_tensor =
        [graph minimumWithPrimaryTensor:max_zero
                        secondaryTensor:one
                                   name:@"hardsigmoid_result"];

    dev_ctx.template Alloc<T>(out);

    id<MTLBuffer> out_buffer = backends::mps::GetMTLBuffer(*out);
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for hardsigmoid";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for hardsigmoid";
      return;
    }
...
void HardSigmoidKernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    HardSigmoidKernelImpl<T>(*mps_ctx, x, slope, offset, out);
  } else {
