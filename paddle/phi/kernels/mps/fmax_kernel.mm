...
template <typename T>
void FMaxKernelImpl(const MPSContext& dev_ctx,
                    const DenseTensor& x,
                    const DenseTensor& y,
                    DenseTensor* out) {
  @autoreleasepool {
    ...
    // fmax(x, y): elementwise maximum ignoring NaN operands (std::fmax).
    // MPSGraph's plain maximum matches this NaN-suppressing behavior.
    MPSGraphTensor* result_tensor = [graph maximumWithPrimaryTensor:x_tensor
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for fmax";
      return;
    }
    ...
    if (x_buffer == nil || y_buffer == nil) {
      VLOG(3) << "Input buffers not available, using CPU fallback for fmax";
      return;
    }
...
void FMaxKernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    FMaxKernelImpl<T>(*mps_ctx, x, y, out);
  } else {
