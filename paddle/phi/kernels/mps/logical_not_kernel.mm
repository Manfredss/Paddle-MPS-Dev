...
template <typename T>
void LogicalNotKernelImpl(const MPSContext& dev_ctx,
                          const DenseTensor& x,
                          DenseTensor* out) {
  @autoreleasepool {
    ...
    MPSGraphTensor* result_tensor =
        [graph notWithTensor:x_tensor name:@"logical_not_result"];

    dev_ctx.template Alloc<bool>(out);

    id<MTLBuffer> out_buffer = backends::mps::GetMTLBuffer(*out);
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for logical_not";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for logical_not";
      return;
    }
...
void LogicalNotKernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    LogicalNotKernelImpl<T>(*mps_ctx, x, out);
  } else {
...
PD_REGISTER_KERNEL(logical_not,
                   MPS,
                   ALL_LAYOUT,
                   phi::LogicalNotKernel,
                   bool) {
