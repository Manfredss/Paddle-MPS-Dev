...
template <typename T>
void HardTanhKernelImpl(const MPSContext& dev_ctx,
                        const DenseTensor& x,
                        float t_min,
                        float t_max,
                        DenseTensor* out) {
  @autoreleasepool {
    ...
    // hardtanh(x) = min(max(x, t_min), t_max)
    MPSGraphTensor* t_min_tensor =
        [graph constantWithScalar:static_cast<float>(t_min)
                            shape:@[@1]
                         dataType:backends::mps::GetMPSDataType(x.dtype())];
    MPSGraphTensor* t_max_tensor =
        [graph constantWithScalar:static_cast<float>(t_max)
                            shape:@[@1]
                         dataType:backends::mps::GetMPSDataType(x.dtype())];

    MPSGraphTensor* max_t_min =
        [graph maximumWithPrimaryTensor:x_tensor
                        secondaryTensor:t_min_tensor
                                   name:@"hardtanh_max_t_min"];
    MPSGraphTensor* result_tensor =
        [graph minimumWithPrimaryTensor:max_t_min
                        secondaryTensor:t_max_tensor
                                   name:@"hardtanh_result"];

    dev_ctx.template Alloc<T>(out);

    id<MTLBuffer> out_buffer = backends::mps::GetMTLBuffer(*out);
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for hardtanh";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for hardtanh";
      return;
    }
...
void HardTanhKernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    HardTanhKernelImpl<T>(*mps_ctx, x, t_min, t_max, out);
  } else {
