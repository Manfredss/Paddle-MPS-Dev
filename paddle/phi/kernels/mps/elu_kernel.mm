...
template <typename T>
void EluKernelImpl(const MPSContext& dev_ctx,
                   const DenseTensor& x,
                   float alpha,
                   DenseTensor* out) {
  @autoreleasepool {
    ...
    MPSGraphTensor* alpha_t = [graph constantWithScalar:alpha
                                                  shape:@[@1]
                                               dataType:MPSDataTypeFloat32];
    MPSGraphTensor* exp_x = [graph exponentWithTensor:x_tensor name:@"elu_exp"];
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for elu";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for elu";
      return;
    }
...
void EluKernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    EluKernelImpl<T>(*mps_ctx, x, alpha, out);
  } else {
