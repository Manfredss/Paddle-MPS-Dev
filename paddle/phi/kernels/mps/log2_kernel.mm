...
template <typename T>
void Log2KernelImpl(const MPSContext& dev_ctx,
                    const DenseTensor& x,
                    DenseTensor* out) {
  @autoreleasepool {
    ...
    // y = log2(x) using MPSGraph logarithmBase2WithTensor:
    MPSGraphTensor* result_tensor = [graph logarithmBase2WithTensor:x_tensor
                                                               name:@"log2_result"];
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for log2";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for log2";
      return;
    }
...
void Log2Kernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    Log2KernelImpl<T>(*mps_ctx, x, out);
  } else {
