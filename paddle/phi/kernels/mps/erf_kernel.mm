...
template <typename T>
void ErfKernelImpl(const MPSContext& dev_ctx,
                   const DenseTensor& x,
                   DenseTensor* out) {
  @autoreleasepool {
    ...
    // y = erf(x) using MPSGraph erfWithTensor:
    MPSGraphTensor* result_tensor = [graph erfWithTensor:x_tensor
                                                    name:@"erf_result"];
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for erf";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for erf";
      return;
    }
