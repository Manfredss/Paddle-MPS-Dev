...
template <typename T>
void AsinKernelImpl(const MPSContext& dev_ctx,
                    const DenseTensor& x,
                    DenseTensor* out) {
  @autoreleasepool {
    ...
    // y = asin(x) using MPSGraph asinWithTensor:
    MPSGraphTensor* result_tensor = [graph asinWithTensor:x_tensor
                                                     name:@"asin_result"];
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for asin";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for asin";
      return;
    }
...
void AsinKernel(const Context& dev_ctx,
                const DenseTensor& x,
                DenseTensor* out) {
  if (x.numel() == 0) {
    dev_ctx.template Alloc<T>(out);
  ...
  if (mps_ctx != nullptr) {
    AsinKernelImpl<T>(*mps_ctx, x, out);
  } else {
