...
template <typename T>
void LeakyReluKernelImpl(const MPSContext& dev_ctx,
                         const DenseTensor& x,
                         double alpha,
                         DenseTensor* out) {
  @autoreleasepool {
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for leaky_relu";
      return;
    }
...
void LeakyReluKernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    LeakyReluKernelImpl<T>(*mps_ctx, x, alpha, out);
  } else {
