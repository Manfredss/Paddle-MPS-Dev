...
template <typename T>
void LogicalAndKernelImpl(const MPSContext& dev_ctx,
                          const DenseTensor& x,
                          const DenseTensor& y,
                          DenseTensor* out) {
  @autoreleasepool {
    ...
    MPSGraphTensor* result_tensor =
        [graph logicalANDWithPrimaryTensor:x_tensor
                          secondaryTensor:y_tensor
                                     name:@"logical_and_result"];

    dev_ctx.template Alloc<bool>(out);

    id<MTLBuffer> out_buffer = backends::mps::GetMTLBuffer(*out);
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for logical_and";
      return;
    }
    ...
    if (x_buffer == nil || y_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for logical_and";
      return;
    }
...
void LogicalAndKernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    LogicalAndKernelImpl<T>(*mps_ctx, x, y, out);
  } else {
