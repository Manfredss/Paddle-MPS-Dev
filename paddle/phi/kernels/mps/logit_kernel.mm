...
template <typename T>
void LogitKernelImpl(const MPSContext& dev_ctx,
                     const DenseTensor& x,
                     double eps,
                     DenseTensor* out) {
  @autoreleasepool {
    ...
    // logit(x) = log(x / (1 - x)); when eps > 0 the input is first clamped
    // to [eps, 1 - eps]. With eps == 0 out-of-domain inputs naturally
    // produce nan/inf.
    MPSGraphTensor* one = [graph constantWithScalar:1.0f
                                              shape:@[@1]
                                           dataType:backends::mps::GetMPSDataType(x.dtype())];
    MPSGraphTensor* input_tensor = x_tensor;
    if (eps > 0.0) {
      MPSGraphTensor* eps_tensor =
          [graph constantWithScalar:static_cast<float>(eps)
                              shape:@[@1]
                           dataType:backends::mps::GetMPSDataType(x.dtype())];
      MPSGraphTensor* one_minus_eps_tensor =
          [graph constantWithScalar:static_cast<float>(1.0 - eps)
                              shape:@[@1]
                           dataType:backends::mps::GetMPSDataType(x.dtype())];
      input_tensor = [graph clampWithTensor:x_tensor
                             minValueTensor:eps_tensor
                             maxValueTensor:one_minus_eps_tensor
                                       name:@"logit_clamped"];
    }
    MPSGraphTensor* one_minus_x =
        [graph subtractionWithPrimaryTensor:one
                            secondaryTensor:input_tensor
                                       name:@"logit_one_minus_x"];
    MPSGraphTensor* ratio =
        [graph divisionWithPrimaryTensor:input_tensor
                         secondaryTensor:one_minus_x
                                    name:@"logit_ratio"];
    MPSGraphTensor* result_tensor = [graph logarithmWithTensor:ratio
                                                          name:@"logit_result"];
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for logit";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for logit";
      return;
    }
...
void LogitKernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    LogitKernelImpl<T>(*mps_ctx, x, eps, out);
  } else {
