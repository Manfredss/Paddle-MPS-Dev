...
template <typename T>
void AtanhKernelImpl(const MPSContext& dev_ctx,
                     const DenseTensor& x,
                     DenseTensor* out) {
  @autoreleasepool {
    ...
    // y = atanh(x) = 0.5 * log((1 + x) / (1 - x)); domain requires |x| < 1.
    MPSGraphTensor* result_tensor = [graph atanhWithTensor:x_tensor
                                                      name:@"atanh_result"];
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for atanh";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for atanh";
      return;
    }
...
void AtanhKernel(const Context& dev_ctx,
                 const DenseTensor& x,
                 DenseTensor* out) {
  if (x.numel() == 0) {
    dev_ctx.template Alloc<T>(out);
  ...
  if (mps_ctx != nullptr) {
    AtanhKernelImpl<T>(*mps_ctx, x, out);
  } else {
