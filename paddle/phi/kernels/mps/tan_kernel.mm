...
template <typename T>
void TanKernelImpl(const MPSContext& dev_ctx,
                   const DenseTensor& x,
                   DenseTensor* out) {
  @autoreleasepool {
    ...
    // y = tan(x) using MPSGraph tanWithTensor:
    MPSGraphTensor* result_tensor = [graph tanWithTensor:x_tensor
                                                    name:@"tan_result"];
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for tan";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for tan";
      return;
    }
...
void TanKernel(const Context& dev_ctx,
               const DenseTensor& x,
               DenseTensor* out) {
  if (x.numel() == 0) {
    dev_ctx.template Alloc<T>(out);
