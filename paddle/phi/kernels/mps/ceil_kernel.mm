...
template <typename T>
void CeilKernelImpl(const MPSContext& dev_ctx,
                    const DenseTensor& x,
                    DenseTensor* out) {
  @autoreleasepool {
    ...
    // y = ceil(x) using MPSGraph ceilWithTensor:
    MPSGraphTensor* result_tensor = [graph ceilWithTensor:x_tensor
                                                     name:@"ceil_result"];
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS output buffer not available for ceil; skipping graph run";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "MPS input buffer not available for ceil; skipping graph run";
      return;
    }
...
void CeilKernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    CeilKernelImpl<T>(*mps_ctx, x, out);
  } else {
