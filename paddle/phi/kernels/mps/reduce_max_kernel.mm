...
template <typename T>
void MaxKernelImpl(const MPSContext& dev_ctx,
                   const DenseTensor& x,
                   const IntArray& dims,
                   bool keep_dim,
                   DenseTensor* out) {
  @autoreleasepool {
    ...
    MPSGraphTensor* reduced = [graph reductionMaximumWithTensor:x_tensor
                                                           axes:axes
                                                           name:@"max_reduce"];
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for max";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for max";
      return;
    }
...
void MaxKernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    MaxKernelImpl<T>(*mps_ctx, x, dims, keep_dim, out);
  } else {
