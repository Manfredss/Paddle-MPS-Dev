...
void HeavisideKernelImpl(const MPSContext& dev_ctx,
  ...
  @autoreleasepool {
    ...
    // heaviside(x, y) = 0 if x < 0, 1 if x > 0, y if x == 0.
    ...
    MPSGraphTensor* one = [graph constantWithScalar:1.0f
                                              shape:@[@1]
                                           dataType:MPSDataTypeFloat32];
    MPSGraphTensor* is_pos = [graph greaterThanWithPrimaryTensor:x_tensor
                                                secondaryTensor:zero
                                                           name:@"heaviside_is_pos"];
    MPSGraphTensor* step = [graph selectWithPredicateTensor:is_pos
                                       truePredicateTensor:one
                                      falsePredicateTensor:zero
                                                      name:@"heaviside_step"];
    MPSGraphTensor* is_zero = [graph equalWithPrimaryTensor:x_tensor
                                           secondaryTensor:zero
                                                      name:@"heaviside_is_zero"];
    MPSGraphTensor* result_tensor =
        [graph selectWithPredicateTensor:is_zero
    ...
    if (x_buffer == nil || y_buffer == nil) {
      VLOG(3) << "Input buffers not available, using CPU fallback for heaviside";
      return;
    }
