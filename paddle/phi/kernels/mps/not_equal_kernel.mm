...
template <typename T>
void NotEqualKernelImpl(const MPSContext& dev_ctx,
                        const DenseTensor& x,
                        const DenseTensor& y,
                        DenseTensor* out) {
  @autoreleasepool {
    ...
    MPSGraphTensor* result_tensor =
        [graph notEqualWithPrimaryTensor:x_tensor
                          secondaryTensor:y_tensor
                                     name:@"not_equal_result"];

    dev_ctx.template Alloc<bool>(out);

    id<MTLBuffer> out_buffer = backends::mps::GetMTLBuffer(*out);
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for not_equal";
      return;
    }
    ...
    if (x_buffer == nil || y_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for not_equal";
      return;
    }
...
void NotEqualKernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    NotEqualKernelImpl<T>(*mps_ctx, x, y, out);
  } else {
