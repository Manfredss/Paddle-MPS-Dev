...
template <typename T>
void FloorKernelImpl(const MPSContext& dev_ctx,
                     const DenseTensor& x,
                     DenseTensor* out) {
  @autoreleasepool {
    ...
    // y = floor(x) using MPSGraph floorWithTensor:
    MPSGraphTensor* result_tensor = [graph floorWithTensor:x_tensor
                                                      name:@"floor_result"];
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS output buffer not available for floor; skipping graph run";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "MPS input buffer not available for floor; skipping graph run";
      return;
    }
...
void FloorKernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    FloorKernelImpl<T>(*mps_ctx, x, out);
  } else {
