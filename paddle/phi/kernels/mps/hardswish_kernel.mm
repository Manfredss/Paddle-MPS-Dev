...
template <typename T>
void HardSwishKernelImpl(const MPSContext& dev_ctx,
                         const DenseTensor& x,
                         DenseTensor* out) {
  @autoreleasepool {
    ...
    // hard_swish(x) = x * min(max(x + 3, 0), 6) / 6
    // (offset=3, threshold=6, scale=6 are hard-coded defaults in phi)
    MPSGraphTensor* zero = [graph constantWithScalar:0.0f
                                               shape:@[@1]
                                            dataType:backends::mps::GetMPSDataType(x.dtype())];
    MPSGraphTensor* three = [graph constantWithScalar:3.0f
                                                shape:@[@1]
                                             dataType:backends::mps::GetMPSDataType(x.dtype())];
    ...
    MPSGraphTensor* x_plus_three =
        [graph additionWithPrimaryTensor:x_tensor
                         secondaryTensor:three
                                    name:@"hardswish_x_plus_three"];
    MPSGraphTensor* max_zero =
        [graph maximumWithPrimaryTensor:x_plus_three
                        secondaryTensor:zero
                                   name:@"hardswish_max_zero"];
    MPSGraphTensor* clamped =
        [graph minimumWithPrimaryTensor:max_zero
                        secondaryTensor:six
                                   name:@"hardswish_clamped"];
    MPSGraphTensor* x_clamped =
        [graph multiplicationWithPrimaryTensor:x_tensor
                               secondaryTensor:clamped
                                          name:@"hardswish_x_clamped"];
    MPSGraphTensor* result_tensor =
        [graph divisionWithPrimaryTensor:x_clamped
                         secondaryTensor:six
                                    name:@"hardswish_result"];

    dev_ctx.template Alloc<T>(out);

    id<MTLBuffer> out_buffer = backends::mps::GetMTLBuffer(*out);
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for hardswish";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for hardswish";
      return;
    }
...
void HardSwishKernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    HardSwishKernelImpl<T>(*mps_ctx, x, out);
  } else {
