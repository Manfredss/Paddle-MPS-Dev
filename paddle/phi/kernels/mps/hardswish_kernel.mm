...
void HardSwishKernelImpl(const MPSContext& dev_ctx,
  ...
  @autoreleasepool {
    ...
    // (threshold=6, scale=6, offset=3, matching phi::HardSwishKernel).
    ...
    MPSGraphTensor* six = [graph constantWithScalar:6.0f
                                              shape:@[@1]
                                           dataType:MPSDataTypeFloat32];
    MPSGraphTensor* shifted = [graph additionWithPrimaryTensor:x_tensor
                                              secondaryTensor:three
                                                         name:@"hardswish_add"];
    MPSGraphTensor* clamped_low = [graph maximumWithPrimaryTensor:shifted
                                                 secondaryTensor:zero
                                                            name:@"hardswish_max"];
    MPSGraphTensor* clamped = [graph minimumWithPrimaryTensor:clamped_low
                                             secondaryTensor:six
                                                        name:@"hardswish_min"];
    MPSGraphTensor* prod = [graph multiplicationWithPrimaryTensor:clamped
                                                 secondaryTensor:x_tensor
                                                            name:@"hardswish_mul"];
    MPSGraphTensor* result_tensor = [graph divisionWithPrimaryTensor:prod
                                                    secondaryTensor:six
