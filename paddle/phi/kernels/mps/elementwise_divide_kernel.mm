...
template <typename T>
void DivideKernelImpl(const MPSContext& dev_ctx,
                      const DenseTensor& x,
                      const DenseTensor& y,
                      DenseTensor* out) {
  @autoreleasepool {
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for divide";
      return;
    }
    ...
    if (x_buffer == nil || y_buffer == nil) {
      VLOG(3) << "Input buffers not available, using CPU fallback for divide";
      return;
    }
...
void DivideKernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    DivideKernelImpl<T>(*mps_ctx, x, y, out);
  } else {
