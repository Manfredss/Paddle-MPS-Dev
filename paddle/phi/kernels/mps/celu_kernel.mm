...
void CeluKernelImpl(const MPSContext& dev_ctx,
  ...
  @autoreleasepool {
    ...
    MPSGraphTensor* one = [graph constantWithScalar:1.0f
                                              shape:@[@1]
                                           dataType:backends::mps::GetMPSDataType(x.dtype())];
    MPSGraphTensor* x_over_alpha =
        [graph divisionWithPrimaryTensor:x_tensor
                         secondaryTensor:alpha_tensor
                                    name:@"celu_x_over_alpha"];
    MPSGraphTensor* exp_t = [graph exponentWithTensor:x_over_alpha
                                                 name:@"celu_exp"];
    MPSGraphTensor* exp_minus_one =
        [graph subtractionWithPrimaryTensor:exp_t
                            secondaryTensor:one
                                       name:@"celu_exp_minus_one"];
    MPSGraphTensor* neg_branch =
        [graph multiplicationWithPrimaryTensor:alpha_tensor
                               secondaryTensor:exp_minus_one
                                          name:@"celu_neg_branch"];
    MPSGraphTensor* pred =
