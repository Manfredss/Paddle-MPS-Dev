...
template <typename T>
void CeluKernelImpl(const MPSContext& dev_ctx,
                    const DenseTensor& x,
                    float alpha,
                    DenseTensor* out) {
  @autoreleasepool {
    ...
    MPSGraphTensor* one = [graph constantWithScalar:1.0f
                                              shape:@[@1]
                                           dataType:MPSDataTypeFloat32];
    MPSGraphTensor* alpha_t = [graph constantWithScalar:alpha
                                                  shape:@[@1]
                                               dataType:MPSDataTypeFloat32];
    MPSGraphTensor* x_div = [graph divisionWithPrimaryTensor:x_tensor
                                            secondaryTensor:alpha_t
                                                       name:@"celu_div"];
    MPSGraphTensor* exp_x = [graph exponentWithTensor:x_div name:@"celu_exp"];
    MPSGraphTensor* exp_m1 = [graph subtractionWithPrimaryTensor:exp_x
                                                secondaryTensor:one
                                                           name:@"celu_expm1"];
    MPSGraphTensor* neg_branch = [graph multiplicationWithPrimaryTensor:alpha_t
                                                       secondaryTensor:exp_m1
                                                                  name:@"celu_neg"];
    MPSGraphTensor* is_neg = [graph lessThanWithPrimaryTensor:x_tensor
                                            secondaryTensor:zero
                                                       name:@"celu_is_neg"];
    MPSGraphTensor* result_tensor =
        [graph selectWithPredicateTensor:is_neg
                     truePredicateTensor:neg_branch
                    falsePredicateTensor:x_tensor
                                    name:@"celu_result"];
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for celu";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for celu";
      return;
    }
...
void CeluKernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    CeluKernelImpl<T>(*mps_ctx, x, alpha, out);
  } else {
