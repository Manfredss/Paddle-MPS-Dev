...
template <typename T>
void CoshKernelImpl(const MPSContext& dev_ctx,
                    const DenseTensor& x,
                    DenseTensor* out) {
  @autoreleasepool {
    ...
    // y = cosh(x) using MPSGraph coshWithTensor:
    MPSGraphTensor* result_tensor = [graph coshWithTensor:x_tensor
                                                     name:@"cosh_result"];
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for cosh";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for cosh";
      return;
    }
...
void CoshKernel(const Context& dev_ctx,
                const DenseTensor& x,
                DenseTensor* out) {
  if (x.numel() == 0) {
    dev_ctx.template Alloc<T>(out);
  ...
  if (mps_ctx != nullptr) {
    CoshKernelImpl<T>(*mps_ctx, x, out);
  } else {
