...
void ThresholdedReluKernelImpl(const MPSContext& dev_ctx,
  ...
  @autoreleasepool {
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for thresholded_relu";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for thresholded_relu";
      return;
    }
