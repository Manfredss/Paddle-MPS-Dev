...
template <typename T>
void ThresholdedReluKernelImpl(const MPSContext& dev_ctx,
                               const DenseTensor& x,
                               float threshold,
                               float value,
                               DenseTensor* out) {
  @autoreleasepool {
    ...
    // thresholded_relu(x) = x if x > threshold, else value
...
void ThresholdedReluKernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    ThresholdedReluKernelImpl<T>(*mps_ctx, x, threshold, value, out);
  } else {
...
PD_REGISTER_KERNEL(thresholded_relu,
                   MPS,
                   ALL_LAYOUT,
