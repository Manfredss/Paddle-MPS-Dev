...
template <typename T>
void ReciprocalKernelImpl(const MPSContext& dev_ctx,
                          const DenseTensor& x,
                          DenseTensor* out) {
  @autoreleasepool {
    ...
    // y = 1 / x using MPSGraph reciprocalWithTensor:
    MPSGraphTensor* result_tensor = [graph reciprocalWithTensor:x_tensor
                                                           name:@"reciprocal_result"];

    dev_ctx.template Alloc<T>(out);

    id<MTLBuffer> out_buffer = backends::mps::GetMTLBuffer(*out);
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for reciprocal";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for reciprocal";
      return;
    }
...
void ReciprocalKernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    ReciprocalKernelImpl<T>(*mps_ctx, x, out);
  } else {
