...
template <typename T>
void AtanKernelImpl(const MPSContext& dev_ctx,
                    const DenseTensor& x,
                    DenseTensor* out) {
  @autoreleasepool {
    ...
    // y = atan(x) using MPSGraph atanWithTensor:
    MPSGraphTensor* result_tensor = [graph atanWithTensor:x_tensor
                                                     name:@"atan_result"];
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for atan";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for atan";
      return;
    }
...
void AtanKernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    AtanKernelImpl<T>(*mps_ctx, x, out);
  } else {
