...
void SoftShrinkKernelImpl(const MPSContext& dev_ctx,
  ...
  @autoreleasepool {
    ...
    MPSGraphTensor* lam = [graph constantWithScalar:lambda
                                              shape:@[@1]
                                           dataType:MPSDataTypeFloat32];
    MPSGraphTensor* neg_lam = [graph constantWithScalar:-lambda
                                                  shape:@[@1]
                                               dataType:MPSDataTypeFloat32];
    MPSGraphTensor* x_minus_lam = [graph subtractionWithPrimaryTensor:x_tensor
                                                     secondaryTensor:lam
                                                                name:@"softshrink_minus"];
    MPSGraphTensor* x_plus_lam = [graph additionWithPrimaryTensor:x_tensor
                                                 secondaryTensor:lam
                                                            name:@"softshrink_plus"];
    MPSGraphTensor* is_low = [graph lessThanWithPrimaryTensor:x_tensor
                                            secondaryTensor:neg_lam
                                                       name:@"softshrink_is_low"];
    MPSGraphTensor* inner = [graph selectWithPredicateTensor:is_low
                                        truePredicateTensor:x_plus_lam
                                       falsePredicateTensor:zero
                                                       name:@"softshrink_inner"];
    MPSGraphTensor* is_high = [graph greaterThanWithPrimaryTensor:x_tensor
                                                secondaryTensor:lam
                                                           name:@"softshrink_is_high"];
    MPSGraphTensor* result_tensor =
        [graph selectWithPredicateTensor:is_high
                     truePredicateTensor:x_minus_lam
                    falsePredicateTensor:inner
