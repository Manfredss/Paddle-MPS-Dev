...
template <typename T>
void ReluKernelImpl(const MPSContext& dev_ctx,
                    const DenseTensor& x,
                    DenseTensor* out) {
  @autoreleasepool {
    ...
    // Perform ReLU using MPSGraph (ReLU = max(x, 0))
    MPSGraphTensor* zero_tensor = [graph constantWithScalar:0.0f
                                                        shape:@[@1]
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for relu";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for relu";
      return;
    }
...
void ReluKernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    ReluKernelImpl<T>(*mps_ctx, x, out);
  } else {
