...
template <typename T>
void Log10KernelImpl(const MPSContext& dev_ctx,
                     const DenseTensor& x,
                     DenseTensor* out) {
  @autoreleasepool {
    ...
    // y = log10(x) using MPSGraph logarithmBase10WithTensor:
    MPSGraphTensor* result_tensor = [graph logarithmBase10WithTensor:x_tensor
                                                                name:@"log10_result"];
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for log10";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for log10";
      return;
    }
...
void Log10Kernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    Log10KernelImpl<T>(*mps_ctx, x, out);
  } else {
