...
void FloorDivideKernelImpl(const MPSContext& dev_ctx,
  ...
  @autoreleasepool {
    ...
    // Floating-point floor division: floor(x / y), matching paddle's
    // FloorDivideFunctor for real inputs.
    MPSGraphTensor* div = [graph divisionWithPrimaryTensor:x_tensor
                                          secondaryTensor:y_tensor
                                                     name:@"floor_divide_div"];
    MPSGraphTensor* result_tensor = [graph floorWithTensor:div
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for floor_divide";
      return;
    }
    ...
    if (x_buffer == nil || y_buffer == nil) {
      VLOG(3) << "Input buffers not available, using CPU fallback for floor_divide";
      return;
    }
