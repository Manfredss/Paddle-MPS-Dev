...
template <typename T>
void Relu6KernelImpl(const MPSContext& dev_ctx,
                     const DenseTensor& x,
                     DenseTensor* out) {
  @autoreleasepool {
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for relu6";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for relu6";
      return;
    }
...
void Relu6Kernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    Relu6KernelImpl<T>(*mps_ctx, x, out);
  } else {
