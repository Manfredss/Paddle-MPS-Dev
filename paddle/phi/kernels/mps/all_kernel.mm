...
template <typename T>
void AllKernelImpl(const MPSContext& dev_ctx,
                   const DenseTensor& x,
  ...
  @autoreleasepool {
    ...
    MPSGraphTensor* x_float = [graph castTensor:x_tensor
                                         toType:MPSDataTypeFloat32
                                           name:@"all_cast"];
    MPSGraphTensor* reduced = [graph reductionMinimumWithTensor:x_float
                                                           axes:axes
                                                           name:@"all_reduce"];
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for all";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for all";
      return;
    }
...
PADDLE_API void AllKernel(const Context& dev_ctx,
                          ...
                          DenseTensor* out) {
  if (x.numel() == 0) {
    // Empty input: fill with true, the identity element of logical AND
    // (matches CPU semantics: np.all of an empty array is True).
    ...
    if (out->numel() > 0) {
      std::fill_n(out_data, out->numel(), true);
    }
    return;
  }
  ...
  if (mps_ctx != nullptr) {
    AllKernelImpl<T>(*mps_ctx, x, dims, keep_dim, out);
  } else {
