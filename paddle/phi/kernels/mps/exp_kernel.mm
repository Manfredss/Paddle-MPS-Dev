...
template <typename T>
void ExpKernelImpl(const MPSContext& dev_ctx,
                   const DenseTensor& x,
                   DenseTensor* out) {
  @autoreleasepool {
    ...
    // Perform exponential using MPSGraph
    MPSGraphTensor* result_tensor = [graph exponentWithTensor:x_tensor
                                                         name:@"exp_result"];
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for exp";
      return;
    }
    ...
    if (x_buffer == nil) {
      VLOG(3) << "Input buffer not available, using CPU fallback for exp";
      return;
    }
