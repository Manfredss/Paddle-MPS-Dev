...
template <typename T>
void MinimumKernelImpl(const MPSContext& dev_ctx,
                       const DenseTensor& x,
                       const DenseTensor& y,
                       DenseTensor* out) {
  @autoreleasepool {
    ...
    // Element-wise minimum (with broadcasting), matches torch.minimum / np.minimum
    ...
    if (out_buffer == nil) {
      VLOG(3) << "MPS buffer not available, using CPU fallback for minimum";
      return;
    }
    ...
    if (x_buffer == nil || y_buffer == nil) {
      VLOG(3) << "Input buffers not available, using CPU fallback for minimum";
      return;
    }
...
void MinimumKernel(const Context& dev_ctx,
  ...
  if (mps_ctx != nullptr) {
    MinimumKernelImpl<T>(*mps_ctx, x, y, out);
  } else {
