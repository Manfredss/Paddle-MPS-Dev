...
class TestCoshMPS(unittest.TestCase):
    """MPS-backend coverage for paddle.cosh."""
    ...
    def _check_vs_numpy(self, x_np, rtol=1e-4, atol=1e-4):
        out = paddle.cosh(paddle.to_tensor(x_np, place="mps")).numpy()
        np.testing.assert_allclose(out, np.cosh(x_np), rtol=rtol, atol=atol)

    def _check_vs_cpu(self, x_np, rtol=1e-5, atol=1e-5):
        out_mps = paddle.cosh(paddle.to_tensor(x_np, place="mps")).numpy()
        out_cpu = paddle.cosh(paddle.to_tensor(x_np, place="cpu")).numpy()
    ...
    def test_cosh_is_even_function(self):
        x = np.random.uniform(-3.0, 3.0, (4, 5)).astype(np.float32)
        x_p = paddle.to_tensor(x, place="mps")
        neg_p = paddle.to_tensor(-x, place="mps")
        np.testing.assert_allclose(
            paddle.cosh(x_p).numpy(),
            paddle.cosh(neg_p).numpy(),
            rtol=1e-5, atol=1e-6,
        )

    def test_cosh_at_zero(self):
        x = np.zeros((3, 4), dtype=np.float32)
        out = paddle.cosh(paddle.to_tensor(x, place="mps")).numpy()
        np.testing.assert_allclose(out, np.ones_like(out), rtol=0, atol=1e-7)

    def test_hyperbolic_identity(self):
        # cosh(x)^2 - sinh(x)^2 == 1
        x = np.random.uniform(-2.0, 2.0, (4, 5)).astype(np.float32)
        x_p = paddle.to_tensor(x, place="mps")
        identity = (paddle.cosh(x_p) ** 2 - paddle.sinh(x_p) ** 2).numpy()
        np.testing.assert_allclose(
            identity, np.ones_like(identity), rtol=1e-3, atol=1e-4
        )

    def test_dtype_and_place_preserved(self):
        x = np.random.uniform(-1.0, 1.0, (3, 4)).astype(np.float32)
        out = paddle.cosh(paddle.to_tensor(x, place="mps"))
